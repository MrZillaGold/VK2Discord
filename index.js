    ...
    axios.get(`https://api.vk.com/method/wall.get?owner_id=${id}&count=2&extended=1&access_token=${token}&v=5.103${config.filter && "&filter=owner"}`)
        .then(async res => {
            ...
            if (res.data.response.items.length > 0) {
                ...
                if (news.last_post !== data.date && !(news.published_posts.includes(data.date)) && checkTextOnKeywords(process.env.KEYWORDS ? process.env.KEYWORDS.split(",") : config.keywords, data.text)) {
